...
class AsistenciaBajarItemTarAdapter(var lstAsistencia:ArrayList<Asistencia>? = null, var c: Context?=null)
    : RecyclerView.Adapter<AsistenciaBajarItemTarAdapter.ViewHolder>()
{
                ...
                if(items.ascenso_t=="1" && items.descenso_t=="1"){
                    ...
                    if(c is AsistenciaTarDropActivity){
                        ...
                        AlertDialog.Builder(c)
                            ...
                            .setMessage(
                                "¿Desea reiniciar la bajada de ${items.nombre}?"
                            )
    ...
    fun alumnoNoAsiste(id_alumno:String, id_ruta:String){
        ...
        CoroutineScope(Dispatchers.Main).launch {
            (c as AsistenciaTarDropActivity).getAsistencia(id_ruta)
        }
    }
    ...
    fun alumnoReiniciaAsistencia(id_alumno:String, id_ruta:String){
        val db = TransporteDB.getInstance(c!!)
        CoroutineScope(Dispatchers.IO).launch {
            db.iAsistenciaDAO.reiniciaBajadaTar(id_ruta,id_alumno,getProcesable())
